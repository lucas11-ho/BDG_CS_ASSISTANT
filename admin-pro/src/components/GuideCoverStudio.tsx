...
function drawBanner(ctx: CanvasRenderingContext2D, banner: CoverDesign["banner"], text: string, family: string) {
  ...
  else roundedRect(ctx, rect.x, rect.y, rect.width, rect.height, banner.shape === "pill" ? rect.height / 2 : (banner.shape === "rounded" || banner.shape === "glass") ? Math.min(20, rect.height / 2) : 2);
  ...
  if (banner.textEnabled && text.trim()) {
    ...
    ctx.fillText(text.trim(), tx, rect.y + rect.height / 2, rect.width - 36); ctx.restore();
  }
}
...
export default function GuideCoverStudio({ locale, direction, title, summary = "", category = "", currentCoverUrl = "", canUpload = false, onGenerated }: Props) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        // Safe defaults keep the designer usable even when platform appearance cannot be loaded.
  ...
  useEffect(() => {
    const onMove = (event: PointerEvent) => {
      ...
      setLayers((current) => current.map((layer) => {
        ...
        if (drag.mode === "resize") {
          const width = Math.max(48, Math.min(1280 - layer.x, drag.start.width + dx));
          const height = Math.max(48, Math.min(720 - layer.y, drag.start.height + dy));
  ...
  const addScreenshot = async (file: File) => {
    try {
      ...
      const layer: CanvasLayer = { id: uid(), kind: "screenshot", name: file.name || "Screenshot", url: uploaded.url, iconKey: "", color: "#ffffff", x: design.template === "screenshot-focus" ? 785 : 760, y: 115, width: 420, height: 500, rotation: 0, opacity: 100, radius: 28, shadow: true, visible: true, locked: false, zIndex: z };
      setLayers((current) => [...current, layer]); setSelectedLayerId(layer.id); message.success("Screenshot added. Drag it directly on the cover to position it.");
  ...
  const moveLayerOrder = (directionValue: 1 | -1) => {
    if (!selectedLayer) return;
    patchLayer(selectedLayer.id, { zIndex: selectedLayer.zIndex + directionValue });
  };
  ...
  const saveWorkspace = async () => {
    ...
    try {
      ...
      message.success("Editable cover layout saved. You can reopen it later with Load saved layout.");
  ...
  const generate = async () => {
    ...
    try {
      ...
      ctx.globalAlpha = design.overlay / 100; ctx.fillStyle = design.accentColor; ctx.beginPath(); ctx.arc(1120, 105, 240, 0, Math.PI * 2); ctx.fill(); ctx.beginPath(); ctx.arc(1170, 630, design.template === "minimal" ? 150 : 280, 0, Math.PI * 2); ctx.fill(); ctx.globalAlpha = 1;
      if (design.template === "security-notice") { ctx.globalAlpha = 0.12; ctx.fillStyle = "#ffffff"; roundedRect(ctx, rtl ? 70 : 760, 95, 430, 530, 44); ctx.fill(); ctx.globalAlpha = 1; }
      ...
      const titleAlign = design.titleStyle.align || (rtl ? "right" : "left"); ctx.textAlign = titleAlign; ctx.textBaseline = "alphabetic";
      const textX = titleAlign === "center" ? 640 : titleAlign === "right" ? 1190 : 90; let cursorY = design.textTop;
      ...
      ctx.font = `${design.titleStyle.fontWeight} ${fitted.size}px ${titleFamily}`; ctx.fillStyle = design.titleStyle.color; const lineHeight = Math.round(fitted.size * 1.14);
      for (const line of fitted.lines) { ctx.fillText(line, textX, cursorY, design.maxTextWidth); cursorY += lineHeight; }
      if (subtitle.trim()) {
        cursorY += 20; const summaryAlign = design.subtitleStyle.align || titleAlign; ctx.textAlign = summaryAlign; const summaryX = summaryAlign === "center" ? 640 : summaryAlign === "right" ? 1190 : 90;
        ...
        ctx.font = `${design.subtitleStyle.fontWeight} ${fittedSummary.size}px ${subtitleFamily}`; ctx.fillStyle = design.subtitleStyle.color; const summaryHeight = Math.round(fittedSummary.size * 1.42);
      ...
      const blob = await canvasBlob(canvas); const file = new File([blob], `guide-cover-${safeSlug(locale)}-${Date.now()}.png`, { type: "image/png" });
      const uploaded: any = await api.uploadGuideMotion(file); if (!uploaded?.url) throw new Error("Cover upload did not return a URL"); onGenerated(uploaded.url); message.success("Cover generated. Click Save locale to store it with this Guide language.");
  ...
  return <Card size="small" style={{ marginBottom: 16 }} loading={loadingDesigner}>
    <Alert showIcon type="info" style={{ marginBottom: 14 }} message="Advanced Guide Cover Builder v2" description="Upload real screenshots, add built-in or custom icons, then drag and resize them directly on the 16:9 cover. Keyboard: arrows move 1 px, Shift + arrows move 10 px, Delete removes the selected unlocked layer. iOS typography and multilingual font fallbacks remain built in." />

    <Space wrap style={{ width: "100%", marginBottom: 14 }}>
      <Select value={design.template} onChange={(value) => patchDesign({ template: value })} options={TEMPLATE_OPTIONS} style={{ minWidth: 200 }} />
    ...
    <Row gutter={14} align="top">
      ...
      <Col xs={24} xl={11}>
        <Text strong>Live 16:9 canvas · drag screenshots and icons directly</Text>
        ...
        <div style={{ color: "#8ea0bd", fontSize: 12, marginTop: 8 }}>Export is always 1280 × 720. Center guides appear while dragging. Screenshot and icon positions are stored in canvas pixels, so the final export matches the editor.</div>
