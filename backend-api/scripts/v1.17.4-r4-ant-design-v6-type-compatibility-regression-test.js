...
test('Rich Knowledge Editor uses only v6-compatible vertical Divider orientation', () => {
  const verticalDividers = (richEditor.match(/<Divider orientation="vertical"\s*\/>/g) || []).length;
  assert.ok(verticalDividers >= 1);
  assert.ok(!/<Divider[^>]*\borientation=["'](?:left|right|center)["']/i.test(richEditor));
});
