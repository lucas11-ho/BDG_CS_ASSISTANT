...
assert.ok(separationMigration.includes('keep_faq_topic_independent'));
assert.ok(separationMigration.includes('trg_faq_topic_independent'));
assert.ok(separationMigration.includes('CHECK (topic_id IS NULL)'));
assert.ok(separationMigration.includes("NEW.topic := COALESCE(NULLIF(BTRIM(OLD.topic), ''), 'General')"));
